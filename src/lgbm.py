...
from utils import manual_bayes_optimize
...
def add_report_features(df):
    ...
    df["has_hemorrhage"] = df["report"].str.contains(r"hemorrhage|haemorrhage|bleed").astype(int)
    df["has_calcification"] = df["report"].str.contains(r"calcif").astype(int)
    df["has_diffusion_restrict"] = df["report"].str.contains(r"diffusion|restrict").astype(int)
    df["has_necrosis"] = df["report"].str.contains(r"necrosis|necrotic").astype(int)
...
tabular_cols = [c for c in X_train.columns if c != "report"]
...
def tune_class_scales(y_true, proba, focus_ids, rounds=4):
    ...
    grid = np.unique(
        np.concatenate([np.linspace(0.5, 3.0, 26), np.array([1.15, 1.35, 1.65, 2.25, 2.75])])
    )
...
print("Class Scales:", {label_encoder.inverse_transform([i])[0]: float(class_scales[i]) for i in focus_ids})

# train again on test + val
X_full = pd.concat([X_train, X_val], axis=0)
y_full = pd.concat([y_train, y_val], axis=0)
X_full_tab = X_full.drop(columns=["report"])
X_test_tab_final = X_test.drop(columns=["report"])
...
X_test_final = hstack([csr_matrix(X_test_tab_final_encoded.to_numpy(dtype="float32")), X_test_text_final], format="csr")
